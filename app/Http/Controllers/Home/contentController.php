...
class contentController extends Controller
{
    ...
    public function home()
    {
        ...
        $sharing = jadwal_sharing::orderBy('created_at', 'desc')->simplePaginate(1);
        $acara = acara::orderBy('created_at', 'desc')->simplepaginate(1);
    ...
    {
        $footers = Footer::all();
        $sharings = jadwal_sharing::orderBy('created_at', 'desc')->get();
    ...
    public function agenda()
    {
        ...
        $partnership = partnership::all();
        $footers = Footer::all();
        $agendas = acara::orderBy('created_at', 'desc')->get();
    ...
    {
        if($request){
            $sertifikat = sertifikat::where('qrcode', 'LIKE', "%" . $request->cari . "%")->get();
        }
        $footers = Footer::all();
        return view('include-content.sertifikat', compact('footers','sertifikat','request'));
    }

    public function Carisertif(Request $request){
        $cari = $request->cari;
        // mengambil data dari table pegawai sesuai pencarian data
        $sertifikat = sertifikat::where('qrcode', 'nama_peserta', "%" . $cari . "%")->get();
        $footers = Footer::all();
