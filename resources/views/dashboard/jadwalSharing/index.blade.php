                        ...
                        <tbody>
                            @forelse ($sharings as $key => $sharing)
                                <tr>
                                    ...
                                    <td><a class="btn"
                                            href="{{ route('jadwal-sharing.show', $sharing->id) }}">{{ $sharing->tittle }}</a>
                                    </td>
                                    <td>{!! \Illuminate\Support\Str::limit($sharing->deskripsi, 150, '.....')!!} </td>
                                    <td>{{ $sharing->jadwal }}</td>
                                    <td>{{ $sharing->pemateri }}</td>
                                    <td>{{ $sharing->ruangan }}</td>
                                    <td>{{ $sharing->kategori->nama_kategori }}</td>
                                    <td>
                                        <img src="{{ Storage::url($sharing->image) }}" alt="gambar" width="150px"
                                            class="tumbnail img-fluid">
                                    </td>
                                    <td>
                                        <a href="{{ route('jadwal-sharing.edit', $sharing->id) }}"
